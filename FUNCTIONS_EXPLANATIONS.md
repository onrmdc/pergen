...
| `run_config_lines_pty(...)` (modified wave-7) | Same pattern as `run_command`. The interactive PTY config-push helper is the highest-blast-radius leak surface because authentication failures during config push happen mid-session, and pre-wave-7 the paramiko exception string was returned verbatim. **Wave-7.4 deprecated for NX-OS recovery** — see `run_config_lines_shell` below. |
| `run_config_lines_shell(...)` (NEW wave-7.4) | Real interactive shell config push via `client.invoke_shell()`. Used by `interface_recovery.recover_interfaces_cisco_nxos`. Sends each line individually, waits for the device prompt before sending the next, captures the full transcript so the operator can audit what NX-OS actually said. Replaces `run_config_lines_pty` for the recovery path: the old `exec_command(script, get_pty=True)` model only ran the first line of a multi-line script on NX-OS — the wave-7.3 split-into-two-sessions fix was correct but was still calling a runner that did not actually execute the config. |
| `_open_shell_channel(...)` (NEW wave-7.4) | Helper that connects via paramiko and returns the `invoke_shell()` channel. Stashes the underlying client on `chan._pergen_client` so the caller can close both. Extracted as a separate function so tests can monkeypatch it without touching paramiko. |
| `_read_until_prompt(chan, deadline)` (NEW wave-7.4) | Reads from a shell channel until `_CONFIG_PROMPT_RE` matches or `deadline` expires. Returns `(accumulated_text, prompt_seen)`. Includes a 0.5 s grace period after prompt detection to absorb trailing bytes (some NX-OS versions double-print). |
| `_CONFIG_PROMPT_RE` (NEW wave-7.4) | Regex matching NX-OS / IOS prompts: `hostname#`, `hostname(config)#`, `hostname(config-if)#`, `hostname(config-if-Ethernet1/15)#`. Constrained to legitimate hostname chars so a stray `#` in command output cannot fool prompt detection. |
| `_DEVICE_ERROR_RE` (NEW wave-7.4) | Regex matching NX-OS error markers: `% Invalid command`, `% Incomplete`, `% Ambiguous`, `% Cannot...`, `% Error`. When matched against a command's response, the runner returns `"device rejected line: ..."` instead of swallowing the failure. |

Pinned by `tests/test_security_ssh_runner_close_on_exception.py` (8 tests for the legacy paramiko-side hardening) and `tests/test_ssh_runner_shell_config.py` (20 tests for the wave-7.4 interactive shell runner).
