...
def _get_credentials(credential_name: str, secret_key: str, cred_store_module) -> tuple[str, str]:
    ...
    Audit H-3: the credential name is run through ``InputSanitizer``
    before any DB lookup, so log-injection / control-byte names cannot
    reach the SQLite layer (it's parameter-bound today, but the
    sanitiser is the policy-level guard).

    For ``method == "api_key"`` the tuple is ``("", api_key)``; for
    ``method == "basic"`` it is ``(username, password)``. Returns
    ``("", "")`` for an unknown / unsanitisable name.
    """
    from backend.security import InputSanitizer

    name = (credential_name or "").strip()
    if not name:
        # Empty name is "no credential bound" — caller surfaces a friendly
        # error. Don't waste a sanitiser warning on it.
        return "", ""
    ...
    for cmd_cfg in commands:
        ...
        if method == "api":
            ...
            if not cmd:
                ...
                out["commands"].append(entry)
                continue
            # Phase 13: defence in depth — even commands that come from the
            # operator-controlled ``commands.yaml`` go through CommandValidator
            # so a supply-chain compromise of the YAML file cannot bypass
            # the read-only safeguards.
            from backend.security.validator import CommandValidator
            ok, reason = CommandValidator.validate(cmd)
            if not ok:
                entry["error"] = f"rejected by CommandValidator: {reason}"
                out["commands"].append(entry)
                continue
