...
def get_fabrics(devices: list[dict] | None = None) -> list[str]:
    devices = devices or load_inventory()
    fabrics = sorted({d["fabric"] for d in devices if d.get("fabric")})
    return fabrics


def get_sites(fabric: str, devices: list[dict] | None = None) -> list[str]:
    devices = devices or load_inventory()
    sites = sorted({d["site"] for d in devices if d.get("fabric") == fabric and d.get("site")})
    return sites


def get_halls(fabric: str, site: str, devices: list[dict] | None = None) -> list[str]:
    devices = devices or load_inventory()
    if site:
...
def get_roles(fabric: str, site: str, hall: str | None = None, devices: list[dict] | None = None) -> list[str]:
    devices = devices or load_inventory()
    if site:
        subset = [d for d in devices if d.get("fabric") == fabric and d.get("site") == site]
    else:
...
def get_devices_by_tag(tag: str, devices: list[dict] | None = None) -> list[dict]:
...
def save_inventory(devices: list[dict], path: str | None = None) -> None:
